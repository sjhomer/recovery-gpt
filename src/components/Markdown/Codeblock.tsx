...
export const Codeblock: React.FC<CodeblockProps> = ({
  ...
  const toggleExpand = () => {
    ...
    // When we collapse the codeblock, scroll to the top of the codeblock
    let current = codeRef.current as HTMLDivElement
    if (!newState && current) {
      // Find the closest parent pre tag, then look for the previous sibling or nearest parent if missing
      let target = current.closest("pre")?.previousElementSibling || current.closest("pre")?.parentElement as HTMLElement
  ...
  useEffect(() => {
    let code = codeRef?.current?.querySelector("code")
    if (code && code.scrollHeight > 18 * 16) {
  ...
  return (
    <div
      ref={codeRef}
      className={`markdown-codeblock relative z-0 ${
        isExpanded ? "" : isOverflowing && "max-h-72 overflow-hidden"
      }`}
    >
      <div className="flex w-full items-center justify-between bg-zinc-800 px-6 py-2 pr-4 text-zinc-100">
        ...
        <div className="codeblockActions flex items-center space-x-0.5">
          ...
          <CopyToClipboard
            className={{
              container: "relative mt-0 right-6 top-0 lg:top-0 md:right-6 lg:pl-1",
              tooltip: "w-52",
            }}
      ...
      <code {...props} className={cn(`mkd-code`, className)}>
